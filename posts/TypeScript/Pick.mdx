...
- 'T'에서 'K' 프로퍼티만 선택해 새로운 오브젝트 타입을 만드는 내장 제네릭 <code>{`'Pick<T, K>'`}</code>을 이를 사용하지 않고 구현하세요.
...
- 테스트 1: <code>{`MyPick<Todo, 'title'>`}</code>
...
- **결과**: <code>{`{ title: string; }`}</code>

---

- 테스트 2: <code>{`MyPick<Todo, 'title' | 'completed'>`}</code>

```tsx
type Expected2 = { title: string; completed: boolean; };
type Test2 = MyPick<Todo, 'title' | 'completed'>;
// 결과: <code>{`{ title: string; completed: boolean; }`}</code>
...
- 테스트 3: <code>{`MyPick<Todo, 'title' | 'completed' | 'invalid'>`}</code>
