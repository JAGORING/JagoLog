...
- **<code>{`Expect<T extends true>`}</code>**

  - T가 true와 일치하는지 확인한 후, 일치하면 특정 타입을 반환시켜주는 역할을 한다

- **<code>{`Equal<X, Y>`}</code>**

  - <code>{`'<T>() => T extends X ? 1 : 2'`}</code> 제네릭 함수 타입으로 함수 반환 타입이 T extends X 조건에 따라 1 또는 2가 된다.
  - <code>{`'(<T>() => T extends X ? 1 : 2) extends (<T>() => T extends Y ? 1 : 2)'`}</code> 로 서로 확장 관계인지 확인하고 동등한 구조인지에 따라 true/false 값을 반환한다.
...
- **<code>{`NotAny<T>`}</code>**
  ...
  - <code>{`IsAny<T>`}</code>를 바탕으로 동작함
  - T가 any인지 아닌지를 반대로 판단하여, **any가 아니면 true, any라면 false**를 반환

  사실 처음에 풀 때는 단순하게 <code>{`Equal<HelloWorld, string>`}</code> HelloWorld가 string과 같아야 하니까 타입은 string! 으로 생각했다. 하지만 utils 파일에서 어떤 동작이 일어나는지 분석하고 정리하니까 타입을 파악하는 흐름이 훨씬 더 정확해진 것 같다🤗